...
export default function PngToJpgClient() {
    ...
    async (files: File[]) => {
      ...
      for (const file of files) {
        try {
          ...
          const blob = await new Promise<Blob>((resolve, reject) => {
            canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("Conversion failed"))), "image/jpeg", quality / 100);
          });
          const jpgUrl = URL.createObjectURL(blob);
          converted.push({ id: crypto.randomUUID(), originalName: file.name, originalSize: file.size, url: jpgUrl, size: blob.size });
        } catch (e) {
          console.error(e);
      ...
      if (converted.length > 0) {
        toast.success("Conversion Complete!", {
          description: `${converted.length} file${converted.length !== 1 ? "s" : ""} converted to JPG.`,
        });
      }
    },
    [quality]
  ...
  const downloadAll = () => {
    ...
    toast.success("Downloading All", { description: `${results.length} JPG files are being downloaded.` });
  };
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <label className="block text-sm font-medium text-slate-700 mb-2">
          JPG Quality: <span className="text-red-600 font-semibold">{quality}%</span>
        </label>
      ...
      {processing && (
        <div className="rounded-xl border border-slate-200 bg-white p-6 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-slate-600">Converting to JPG…</p>
        </div>
      )}
            ...
            {results.map((r) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-sm font-medium text-slate-700">{r.originalName.replace(/\.png$/i, ".jpg")}</p>
                ...
                <a
                  ...
                  onClick={() => toast.success("Downloading", { description: r.originalName.replace(/\.png$/i, ".jpg") })}
